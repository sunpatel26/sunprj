...
namespace ERP.Areas.Admin.Controllers
{
    ...
    public class RequestTypeController : SettingsController
    {
            ...
            try
            {
                ...
                Action<IGridColumnCollection<RequestTypeMetadata>> columns = c =>
                {
                    ...
                    c.Add()
                        ...
                        .SetWidth(60)
                        .RenderValueAs(o =>
                            string.Format("<a href='RequestTypeModule/Index/{0}' class='btn'>Section</a>", o.RequestTypeID) +
                ...
                PagedDataTable<RequestTypeMetadata> pds = await _requestType.GetAllList(COMPANYID, gridpage.ToInt(), PAGESIZE, search, orderby.RemoveSpace(), sortby == "0" ? "ASC" : "DESC");
        ...
        public JsonResult SaveRequestType(RequestTypeMetadata model)
        {
            try
            {
                _requestType.Save(model.RequestTypeID, model.EntityID, model.Name, model.AccessByRoles, model.AssignedTo, model.IsActive, USERID, COMPANYID);
